...
{
	for (int i = 0; i < g_maxBullets; ++i)
	if (!m_bullets[i] || !m_bullets[i]->IsActive()) // What is this condition checking? if a bullet is fired
	{
		if (m_bullets[i] && !m_bullets[i]->IsActive()) // What is this condition checking? If bullet hits border it is deleted
		{
			delete m_bullets[i];
		}
...
GameState::~GameState()
{
	...
	// Why don't we "delete [] m_bullets;" ? The array is for m_bulklets not a blank 
}